...
public class NoiseGeneratorPerlin extends NoiseGenerator {
	...
	public double generateNoise(double d, double d1, double d2) {
		...
		return lerp(
				d8,
				lerp(d7,
						lerp(d6, grad(permutations[l1], d3, d4, d5),
				...
				lerp(d7,
						lerp(d6,
								...
								grad(permutations[k2 + 1], d3 - 1.0D, d4,
										d5 - 1.0D)),
						lerp(d6,
								grad(permutations[i2 + 1], d3, d4 - 1.0D,
										d5 - 1.0D),
								grad(permutations[l2 + 1], d3 - 1.0D,
										d4 - 1.0D, d5 - 1.0D))));
	}
	...
	public void func_805_a(double ad[], double d, double d1, double d2, int i,
			...
			for (int i4 = 0; i4 < i; i4++) {
				...
				for (int l4 = 0; l4 < k; l4++) {
					...
					double d11 = lerp(
							...
							grad(permutations[l1 + 1], d14 - 1.0D, 0.0D,
									d19 - 1.0D));
			...
			for (int j6 = 0; j6 < k; j6++) {
				...
				for (int i7 = 0; i7 < j; i7++) {
					...
					if (i7 == 0 || k7 != i2) {
						...
						d15 = lerp(
								d22,
								grad(permutations[l2], d20, d26 - 1.0D, d24),
								grad(permutations[l3], d20 - 1.0D, d26 - 1.0D,
										d24));
						d16 = lerp(
								d22,
								grad(permutations[k2 + 1], d20, d26, d24 - 1.0D),
								grad(permutations[k3 + 1], d20 - 1.0D, d26,
										d24 - 1.0D));
						d18 = lerp(
								d22,
								grad(permutations[l2 + 1], d20, d26 - 1.0D,
										d24 - 1.0D),
								grad(permutations[l3 + 1], d20 - 1.0D,
										d26 - 1.0D, d24 - 1.0D));
					}
