	...
	public boolean a(World world, Random random, int i, int j, int k)
    {
        ...
        for(int j1 = 0; j1 < 16; j1++)
        {
            for(int j2 = 0; j2 < 16; j2++)
            {
                for(int j3 = 0; j3 < 8; j3++)
                {
                    ...
                    Material material = world.getMaterial(i + j1, j + j3, k + j2);
                    if(j3 >= 4 && material.isLiquid())
                    {
                        return false;
                    }
                    if(j3 < 4 && !material.isSolid() && world.getTypeId(i + j1, j + j3, k + j2) != field_15235_a)
                    {
                        return false;
                    }
                }
        ...
        for(int k1 = 0; k1 < 16; k1++)
        {
            for(int k2 = 0; k2 < 16; k2++)
            {
                for(int k3 = 0; k3 < 8; k3++)
                {
                    ...
                    {
                        world.setRawTypeId(i + k1, j + k3, k + k2, k3 < 4 ? field_15235_a : 0);
                    }
                }
        ...
        for(int l1 = 0; l1 < 16; l1++)
        {
            for(int l2 = 0; l2 < 16; l2++)
            {
                ...
                {
                    if(aflag[(l1 * 16 + l2) * 8 + l3] && world.getTypeId(i + l1, (j + l3) - 1, k + l2) == Block.DIRT.id && world.b(EnumSkyBlock.SKY, i + l1, j + l3, k + l2) > 0)
                    {
                        world.setRawTypeId(i + l1, (j + l3) - 1, k + l2, Block.GRASS.id);
                    }
                }
            ...
            for(int i2 = 0; i2 < 16; i2++)
            {
                for(int i3 = 0; i3 < 16; i3++)
                {
                    for(int i4 = 0; i4 < 8; i4++)
                    {
                        ...
                        {
                            world.setRawTypeId(i + i2, j + i4, k + i3, Block.STONE.id);
                        }
                    }
