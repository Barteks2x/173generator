...
public class ChunkProviderGenerate extends ChunkGenerator implements IChunkProvider {
    ...
    public void generateTerrain(int i, int j, byte terrainBlockArray[], double ad[]) {
	byte byte0 = 4;
	byte byte1 = 64;
	int k = byte0 + 1;
	byte byte2 = 17;
	int l = byte0 + 1;
	noiseArray = initializeNoiseField(noiseArray, i * byte0, 0, j * byte0, k, byte2, l);
	for (int i1 = 0; i1 < byte0; i1++) {
	    for (int j1 = 0; j1 < byte0; j1++) {
		for (int k1 = 0; k1 < 16; k1++) {
		    double d = 0.125D;
		    double d1 = noiseArray[((i1 + 0) * l + (j1 + 0)) * byte2 + (k1 + 0)];
		    double d2 = noiseArray[((i1 + 0) * l + (j1 + 1)) * byte2 + (k1 + 0)];
		    double d3 = noiseArray[((i1 + 1) * l + (j1 + 0)) * byte2 + (k1 + 0)];
		    double d4 = noiseArray[((i1 + 1) * l + (j1 + 1)) * byte2 + (k1 + 0)];
		    double d5 = (noiseArray[((i1 + 0) * l + (j1 + 0)) * byte2 + (k1 + 1)] - d1) * d;
		    double d6 = (noiseArray[((i1 + 0) * l + (j1 + 1)) * byte2 + (k1 + 1)] - d2) * d;
		    double d7 = (noiseArray[((i1 + 1) * l + (j1 + 0)) * byte2 + (k1 + 1)] - d3) * d;
		    double d8 = (noiseArray[((i1 + 1) * l + (j1 + 1)) * byte2 + (k1 + 1)] - d4) * d;
			...
			for (int i2 = 0; i2 < 4; i2++) {
			    int blockInArray = i2 + i1 * 4 << 11 | 0 + j1 * 4 << 7 | k1 * 8 + l1;
			    char c = '\200';
			    double d14 = 0.25D;
			    double d15 = d10;
	...
	biomesForGeneration = wcm.getBiomeBlock(biomesForGeneration, x * 16, z * 16, 16, 16);
	...
	for (int blockInSectionX = 0; blockInSectionX < 16; blockInSectionX++) {
		...
		for (int section = 0; section < 8; section++) {
			...
			sectionBlocks[section][blockInSectionX|(blockInSectionZ<<4)|(blockInSectionY<<8)] = blockArray[blockInArray++];
		    }
		}
	    }
	}
    ...
    public void replaceBlocksForBiome(int i, int j, byte terrainBlockArray[], BiomeBase abiomebase[]) {
	byte byte0 = 64;
	double d = 0.03125D;
	sandNoise = noiseOctaves4.generateNoiseOctaves(sandNoise, i * 16, j * 16, 0.0D, 16, 16, 1, d, d, 1.0D);
	gravelNoise = noiseOctaves4.generateNoiseOctaves(gravelNoise, i * 16, 109.0134D, j * 16, 16, 1, 16, d, 1.0D, d);
	stoneNoise = noiseOctaves5.generateNoiseOctaves(stoneNoise, i * 16, j * 16, 0.0D, 16, 16, 1, d * 2D, d * 2D, d * 2D);
	for (int k = 0; k < 16; k++) {
	    for (int l = 0; l < 16; l++) {
		BiomeBase biomegenbase = abiomebase[k + l * 16];
		boolean randomSand = sandNoise[k + l * 16] + rand.nextDouble() * 0.20000000000000001D > 0.0D;
		boolean randomGravel = gravelNoise[k + l * 16] + rand.nextDouble() * 0.20000000000000001D > 3D;
	...
	biomesForGeneration = this.wcm.getBiomeBlock(biomesForGeneration, i * 16, j * 16, 16, 16);
	double ad[] = this.wcm.temperature;
	...
	field_4182_g = noiseOctaves6.func_4109_a(field_4182_g, i, k, l, j1, 1.121D, 1.121D, 0.5D);
	field_4181_h = noiseOctaves7.func_4109_a(field_4181_h, i, k, l, j1, 200D, 200D, 0.5D);
	field_4185_d = noiseOctaves3.generateNoiseOctaves(field_4185_d, i, j, k, l, i1, j1, d / 80D, d1 / 160D, d / 80D);
	field_4184_e = noiseOctaves1.generateNoiseOctaves(field_4184_e, i, j, k, l, i1, j1, d, d1, d);
	field_4183_f = noiseOctaves2.generateNoiseOctaves(field_4183_f, i, j, k, l, i1, j1, d, d1, d);
	int k1 = 0;
	int l1 = 0;
	int i2 = 16 / l;
	...
	if (rand.nextInt(8) == 0) {
		...
		(new WorldGenLakes(Block.LAVA.id)).a(worldObj, rand, j1, i5, j8);
	    }
	}
	...
	for (int j2 = 0; j2 < 20; j2++) {
	    ...
	    (new WorldGenMinable(Block.DIRT.id, 32)).a(worldObj, rand, l5, i9, l11);
	}
	...
	for (int k2 = 0; k2 < 10; k2++) {
	    ...
	    (new WorldGenMinable(Block.GRAVEL.id, 32)).a(worldObj, rand, i6, j9, i12);
	}
	...
	for (int i3 = 0; i3 < 20; i3++) {
	    ...
	    (new WorldGenMinable(Block.COAL_ORE.id, 16)).a(worldObj, rand, j6, k9, j12);
	}
	...
	for (int j3 = 0; j3 < 20; j3++) {
	    ...
	    (new WorldGenMinable(Block.IRON_ORE.id, 8)).a(worldObj, rand, k6, l9, k12);
	}
	...
	for (int k3 = 0; k3 < 2; k3++) {
	    ...
	    (new WorldGenMinable(Block.GOLD_ORE.id, 8)).a(worldObj, rand, l6, i10, l12);
	}
	...
	for (int l3 = 0; l3 < 8; l3++) {
	    ...
	    (new WorldGenMinable(Block.REDSTONE_ORE.id, 7)).a(worldObj, rand, i7, j10, i13);
	}
	...
	for (int i4 = 0; i4 < 1; i4++) {
	    ...
	    (new WorldGenMinable(Block.DIAMOND_ORE.id, 7)).a(worldObj, rand, j7, k10, j13);
	}
	...
	for (int j4 = 0; j4 < 1; j4++) {
	    ...
	    (new WorldGenMinable(Block.LAPIS_ORE.id, 6)).a(worldObj, rand, k7, l10, k13);
	}
	...
	for (int i11 = 0; i11 < l7; i11++) {
	    ...
	    worldgenerator.a(worldObj, rand, l13, worldObj.getHighestBlockYAt(l13, j14), j14);
	}
	...
	for (int i14 = 0; i14 < byte0; i14++) {
	    ...
	    (new WorldGenFlowers(Block.YELLOW_FLOWER.id)).a(worldObj, rand, k14, l16, k19);
	}
	...
	for (int l14 = 0; l14 < byte1; l14++) {
	    ...
	    (new WorldGenTallGrass(Block.LONG_GRASS.id, byte2)).a(worldObj, rand, l19, k22, j24);
	}
	...
	for (int i15 = 0; i15 < byte1; i15++) {
	    ...
	    (new WorldGenDeadBush(Block.DEAD_BUSH.id)).a(worldObj, rand, i17, i20, l22);
	}
	...
	if (rand.nextInt(2) == 0) {
	    ...
	    (new WorldGenFlowers(Block.RED_ROSE.id)).a(worldObj, rand, j15, j17, j20);
	}
	if (rand.nextInt(4) == 0) {
	    int k15 = k + rand.nextInt(16) + 8;
	    int k17 = rand.nextInt(128);
	    int k20 = l + rand.nextInt(16) + 8;
	    (new WorldGenFlowers(Block.BROWN_MUSHROOM.id)).a(worldObj, rand, k15, k17, k20);
	}
	if (rand.nextInt(8) == 0) {
	    int l15 = k + rand.nextInt(16) + 8;
	    int l17 = rand.nextInt(128);
	    int l20 = l + rand.nextInt(16) + 8;
	    (new WorldGenFlowers(Block.RED_MUSHROOM.id)).a(worldObj, rand, l15, l17, l20);
	}
	...
	for (int l18 = 0; l18 < 50; l18++) {
	    ...
	    (new WorldGenLiquids(Block.WATER.id)).a(worldObj, rand, l21, k23, l24);
	}
	...
	for (int i19 = 0; i19 < 20; i19++) {
	    ...
	    (new WorldGenLiquids(Block.LAVA.id)).a(worldObj, rand, i22, l23, i25);
	}

	generatedTemperatures = this.wcm.getTemperatures(generatedTemperatures, k + 8, l + 8, 16, 16);
	for (int j19 = k + 8; j19 < k + 8 + 16; j19++) {
		...
		if (d1 < 0.5D && k25 > 0 && k25 < 128 && worldObj.isEmpty(j19, k25, j22) && worldObj.getMaterial(j19, k25 - 1, j22).isSolid() && worldObj.getMaterial(j19, k25 - 1, j22) != Material.ICE) {
