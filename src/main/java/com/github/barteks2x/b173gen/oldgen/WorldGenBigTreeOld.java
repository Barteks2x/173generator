...
public class WorldGenBigTreeOld implements WorldGenerator173 {
    ...
    public boolean canGenerate() {
        ...
        Material id = worldObj.getBlockAt(basePos[0], basePos[1] - 1, basePos[2]).getType();
        if(id != Material.DIRT && id != Material.GRASS) {
            return false;
        }
        int j = a(pos1, pos2);
        if(j == -1) {
    ...
    public void scale(double d, double d1, double d2) {
        m = (int)(d * 12D);
        if(d > 0.5D) {
            n = 5;
        }
    ...
    @SuppressWarnings("cast")
    void a(int i, int j, int k, float f, byte byte0, Material l) {
        ...
        int j1 = -i1;
        int k1 = -i1;
        pos[byte0] = posBase[byte0];
        for(; j1 <= i1; j1++) {
            pos[byte1] = posBase[byte1] + j1;
            for(int l1 = -i1; l1 <= i1;) {
                ...
                if(d > (double)f) {
                    ...
                    pos[byte2] = posBase[byte2] + l1;
                    Material i2 = worldObj.getBlockAt(pos[0], pos[1], pos[2]).getType();
                    if(i2 != Material.AIR && i2 != Material.LEAVES) {
                        l1++;
                    } else {
                        worldObj.getBlockAt(pos[0], pos[1], pos[2]).setType(l);
                        l1++;
                    }
                }
            }
    ...
    void a(int i, int j, int k) {
        ...
        for(int i1 = j + n; l < i1; l++) {
            float f = b(l - j);
            a(i, l, k, f, (byte)1, Material.LEAVES);
        }

    }

    void a(int ai[], int ai1[], Material i) {
        int ai2[] = {0, 0, 0};
        byte byte0 = 0;
        int j = 0;
        for(; byte0 < 3; byte0++) {
        ...
        for(int l = ai2[j] + byte3; k != l; k += byte3) {
            ...
            ai3[byte2] = MathHelper.floor((double)ai[byte2]
                    + (double)k * d1 + 0.5D);
            worldObj.getBlockAt(ai3[0], ai3[1], ai3[2]).setType(i);
        }
