...
public class WorldGenMinableOld extends WorldGenerator173 {

	private int a;
	private int b;

	public WorldGenMinableOld(int i, int j) {
		this.a = i;
		this.b = j;
	}
	...
	public boolean generate(World world, Random random, int i, int j, int k) {
		...
		for (int l = 0; l <= this.b; ++l) {
			...
			double d10 =
					(double)(MathHelper.sin((float)l * 3.1415927F / (float)this.b) + 1.0F) * d9 +
					1.0D;
			double d11 =
					(double)(MathHelper.sin((float)l * 3.1415927F / (float)this.b) + 1.0F) * d9 +
					1.0D;
					...
					for (int l2 = j1; l2 <= i2; ++l2) {
						...
						if (d12 * d12 + d13 * d13 < 1.0D) {
							for (int i3 = k1; i3 <= j2; ++i3) {
								...
								if (d12 * d12 + d13 * d13 + d14 * d14 < 1.0D && world.
										getBlockTypeIdAt(k2, l2, i3) == Material.STONE.getId()) {
									world.getBlockAt(k2, l2, i3).setTypeId(this.a);
								}
							}
						}
					}
