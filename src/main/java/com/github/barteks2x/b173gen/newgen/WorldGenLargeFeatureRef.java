...
public class WorldGenLargeFeatureRef extends StructureGeneratorRef {

    protected final Object WORLD_GEN_LARGE_FEATURE;
    public WorldGenLargeFeatureRef() {
        this.WORLD_GEN_LARGE_FEATURE = newInstance(WorldGenLargeFeature, null, null);
    }

    public boolean generate(World world, Random random, int i, int j) {
        return WorldGenlargeFeature_generate_populator(WORLD_GEN_LARGE_FEATURE, world, random, i, j);
    }

    public void generate(World world, int x, int z, Object[] blocks) {
        WorldGenlargeFeature_generate_generator(WORLD_GEN_LARGE_FEATURE, world, x, z, blocks);
    }
